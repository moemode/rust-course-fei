...
impl ComputerState {
    ...
    fn running_to_sleep(&mut self) {
        self.running = false;
        self.sleeping = true;
        self.idle_time = 0;
    }

    fn sleep_to_running(&mut self) {
        self.running = true;
        self.sleeping = false;
        self.sleep_time = 0;
    }
...
fn pc_transition(mut computer: ComputerState, event: Event) -> ComputerState {
    match event {
        ...
        Event::MoveMouse if computer.running => {
            computer.idle_time = 0;
            computer
        }
        Event::MoveMouse if computer.sleeping => {
            computer.sleep_to_running();
            computer
        }
        Event::PassTime(mut time) if computer.is_on() => {
            computer.up_time += time;
            if computer.running {
                computer.idle_time += time;
            }
            if computer.idle_time > 1000 {
                time = computer.idle_time - 1000;
                computer.running_to_sleep();
            }
